package com.louyj.cottus.client.cmd.server;
...
import com.louyj.cottus.client.cmd.worker.ControlCommand;
...
public class ServerCommand extends BaseCommand {
	...
	public String connect(@ShellOption(value = { "-s",
		...
		}
		{
			if (superAdmin) {
				...
				byte[] encrypted = RsaUtils.encrypt(ClientDetector.CLIENT.identify().getBytes(UTF_8),
						RsaUtils.loadPrivateKey(privateKey));
				password = Base64.encodeBase64String(encrypted);
			}
		}
		ConnectMessage connectMessage = new ConnectMessage(ClientDetector.CLIENT);
	...
	public String servers() {
		ListServersMessage listServersMessage = new ListServersMessage(ClientDetector.CLIENT);
